  ...
  <div class="h-full relative">
    <a-drawer :width="modalWidth" :mask-closable="false" :title="$t('customer.createCustomer')"
      ...
      <div class="relative mt-5">
        <div class="m-5" >
          <a-descriptions v-for="row in customerInfoForm" style="margin-bottom: 25px;"  :title="t(row.itemName)" :column="{ xs: 2, sm: 4, md: 4, lg: 4 }" layout="inline-vertical" bordered>
            <a-descriptions-item v-for="item of row.list" :label="t(item.label)" :span="item.span">
              <span class="content flex items-center">
                <a-select v-if="['phone'].includes(item.type)" default-value="+86" :style="{width:'80px'}" size="small" class="bg-white mr-1 hover:bg-white">
                 <a-option :value="1">+86</a-option>
                </a-select>
                <a-input v-if="['phone','text'].includes(item.type)" :placeholder="t('placeholder.prefix')+t(item.label)" class="bg-white hover:bg-white border-0" />
                <a-radio-group v-if="['radio'].includes(item.type)" :options="options">
                    <template #label="{ data }">
                      <span>{{ t(data.label) }}</span>
                    </template>
                </a-radio-group>
                <a-select v-if="['select'].includes(item.type)" size="small" :placeholder="t('placeholder.prefix')+t(item.label)" class="bg-white  mr-1 hover:bg-white" />
                <a-date-picker v-if="['date'].includes(item.type)" class="bg-white hover:bg-white border-0" />
              </span>
            </a-descriptions-item>
          </a-descriptions>
          <div class="p-5 w-2/3 m-auto">
            <a-button type="primary" class="h-12" shape="round" @click="createCustomer"  long>{{ t('modal.footer.create') }}</a-button>
          </div>
        </div>
